...
class IOTest extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("flatTap success") {
    var counter = 0
    val action = IO{counter += 1; 42}.flatTap(a => IO(println(a)))
    assert(counter == 0)
    val result = action.unsafeRun()
    assert(counter == 1)
    assert(result == 42)
  ...
  test("flatTap failure") {
    val exception = new Exception("Boom")
    var counter = 0
    val action = IO[Unit]{counter += 1; throw exception}.flatTap(a => IO(println(a)))
    assert(counter == 0)
    val result = Try(action.unsafeRun())
    assert(counter == 1)
    assert(result.isFailure)
